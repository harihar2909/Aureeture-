...
type ModalType = 'EDIT_PATH' | 'LAUNCHPAD_INFO' | 'HACKATHON_INFO' | 'JOB_DETAILS';
...
const CareerPathTimeline: FC<{ onOpenModal: (type: ModalType, data?: any) => void, careerGoal: string, careerSteps: PathStep[] }> = ({ onOpenModal, careerGoal, careerSteps }) => (
    <Widget className="p-6 md:p-8 flex flex-col lg:flex-row lg:items-center gap-8">
        ...
        <div className="lg:w-1/3 flex-shrink-0 text-center lg:text-left p-6 bg-primary/10 rounded-xl border border-primary/20">
            ...
            <Button variant="ghost" size="sm" className="mt-2 text-muted-foreground" onClick={() => onOpenModal('EDIT_PATH', { currentGoal: careerGoal })}>
        ...
        <Widget className="p-6 md:p-8">
            ...
            <div className="space-y-4">
                {filteredJobs.length > 0 ? filteredJobs.map(job => {
                    ...
                    return (
                        <div key={job.id} className={`flex items-start justify-between p-4 rounded-lg gap-4 transition-colors ${job.featured ? 'bg-primary/10 border border-primary/20' : 'bg-muted/30'}`}>
                            <div className="flex-grow">
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                                <TooltipProvider><Tooltip><TooltipTrigger asChild>
                                    <Button size="icon" variant="ghost" onClick={() => handleSave(job.id)}>
                                        {isSaved ? <Check className="h-4 w-4 text-green-500" /> : <Bookmark className="h-4 w-4" />}
                                    </Button>
                                </TooltipTrigger><TooltipContent><p>{isSaved ? 'Saved!' : 'Save for later'}</p></TooltipContent></Tooltip></TooltipProvider>
                                <Button size="sm" onClick={() => onOpenModal('JOB_DETAILS', { job })}>Details <ArrowRight className="h-4 w-4 ml-2" /></Button>
                            </div>
                        </div>
                    );
    ...
    return (
        <div className="min-h-screen w-full bg-background text-foreground p-4 md:p-6 lg:p-8 relative overflow-hidden">
            ...
            <Dialog open={modal.type === 'JOB_DETAILS'} onOpenChange={handleCloseModal}>
                <DialogContent>
                    <DialogHeader><DialogTitle>{modal.data?.job?.title}</DialogTitle><DialogDescription>{modal.data?.job?.company} • {modal.data?.job?.category}</DialogDescription></DialogHeader>
                    <p className="mt-4">This is a premier {modal.data?.job?.category.toLowerCase()} opportunity for aspiring entrepreneurs on the Aureeture platform. We are seeking a dynamic individual to join a promising {modal.data?.job?.company.toLowerCase()}.</p>
                </DialogContent>
            </Dialog>
        </div>
    );
