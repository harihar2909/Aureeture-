...
const MentorSessionsPage: React.FC = () => {
  ...
  const mockUpcoming: MentorSession[] = [
    {
      _id: "mock-1",
  ...
  const mockPast: MentorSession[] = [
    {
      ...
      description: "Scalability, cache, and trade-off discussion.",
      ...
      notes: "Strong fundamentals, needs crisper trade-offs.",
    },
    {
      _id: "mock-4",
      mentorId: "mock-mentor",
      ...
      description: "REST vs GraphQL, DB optimization, caching layers.",
      startTime: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000).toISOString(),
      endTime: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000 + 45 * 60 * 1000).toISOString(),
      durationMinutes: 45,
      status: "completed",
      paymentStatus: "paid",
      meetingLink: "https://meet.aureeture.ai/session/arjun-1",
      recordingUrl: "https://recordings.aureeture.ai/arjun-1",
      notes: "Suggested caching and background jobs for heavy tasks.",
    },
  ];
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ message: "Failed to fetch sessions" }));
        throw new Error(errorData.message || `HTTP ${res.status}: Failed to fetch sessions`);
      }
